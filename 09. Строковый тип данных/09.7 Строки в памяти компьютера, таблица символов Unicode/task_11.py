'''
Накручиваем стоимость ответа
Как вы помните из прошлой задачи, 
модератору Сэму за каждый символ его сообщений 
в комментариях Тимур платит в 🐝 (пчёлках-coin) по следующему тарифу:
    <код символа в таблице Unicode>×3🐝
А стоимость всего сообщения складывается из суммы стоимостей всех символов. 
На этот раз Сэму захотелось схитрить 
и попробовать увеличить стоимость своего сообщения, 
заменив в нем некоторые английские буквы на русские. 
Как вы помните, русские буквы в таблице Unicode находятся после английских.
Сэм хочет заменить следующие английские буквы:
    eyopaxcETOPAHXCBM
на соответствующие им русские буквы:
    еуорахсЕТОРАНХСВМ
Тимур визуально разницу не заметит, а Сэм сможет зарабатывать больше пчёлок-coin.
На вход программе подаётся строка текста. 
Требуется написать программу, которая находит стоимость старого 
и нового сообщений Сэма в 🐝 
и выводит текст в следующем формате:
    Старая стоимость: <стоимость старого сообщения>🐝
    Новая стоимость: <стоимость нового сообщения>🐝
На вход программе подаётся строка текста – очередной ответ Сэма в комментариях.
Программа должна вывести текст в соответствии с условием задачи.
Обратите внимание, что если в строке не удаётся заменить буквы, 
то стоимость сообщения не изменится. 😢
'''

# option 1
comment = input()
new_comment = ''
en_chars = 'eyopaxcETOPAHXCBM'
ru_chars = 'еуорахсЕТОРАНХСВМ'
for i in comment:
    if i not in en_chars:
        new_comment += i
    else:
        new_comment += ru_chars[en_chars.find(i)]
old_cost = 0
for i in comment:
    old_cost += ord(i) * 3
print(f"Старая стоимость: {old_cost}🐝")
new_cost = 0
for i in new_comment:
    new_cost += ord(i) * 3
print(f"Новая стоимость: {new_cost}🐝")

# option 2
comment = input()
old_cost = 0
new_cost = 0
en_chars = 'eyopaxcETOPAHXCBM'
ru_chars = 'еуорахсЕТОРАНХСВМ'
for i in comment:
    old_cost += ord(i) * 3
    if i in en_chars:
        en_index = en_chars.index(i)
        ru_index = en_index
        ru_letter = ru_chars[ru_index]
        new_cost += ord(ru_letter) * 3
    else:
        new_cost += ord(i) * 3
print(f'Старая стоимость: {old_cost}🐝')
print(f'Новая стоимость: {new_cost}🐝')